...
import { SearchRequestSchema, validateSchema } from '@/lib/validation';
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const parsedBody = validateSchema(SearchRequestSchema, await request.json(), 'Invalid search request');
  const { query, userId, session_token } = parsedBody;
