...
export async function GET(req: NextRequest) {
  try {
    ...
    // If there is a query, use blended semantic search
    if (query.trim().length > 1) {
      const items = await semanticBlendSearch(query, { city, category, open_now: openNow });
      return NextResponse.json({ items, count: items.length });
    }

    // Fallback: rank_score sorted list with filters
    let q = supabase
      .from('destinations')
      .select('slug, name, city, category, image, rating, reviews_count, rank_score, is_open_now')
      .order('rank_score', { ascending: false })
      .limit(50);

    if (city) q = q.ilike('city', `%${city}%`);
    if (category) q = q.ilike('category', `%${category}%`);
    if (openNow) q = q.eq('is_open_now', true);

    // Editorial integrity prioritization
    const { data, error } = await q;
    if (error) throw error;
    const curated = (data || []).sort((a: any, b: any) => {
    ...
    return NextResponse.json({ items: curated, count: curated.length });
  } catch (e: any) {
