...
import { filtersSchema, parseJsonBody } from '@/lib/utils/validation';
import { z } from 'zod';

const refinePayloadSchema = z.object({
  originalQuery: z
    .string({ invalid_type_error: 'originalQuery must be a string' })
    .trim()
    .min(1, 'originalQuery is required'),
  refinements: z
    .array(z.string().trim().min(1, 'Refinement terms cannot be empty'))
    .default([]),
  allResults: z
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  const { originalQuery, refinements, allResults, filters } = await parseJsonBody(
    request,
    refinePayloadSchema,
    { errorMessage: 'Invalid refine payload' }
  );
  ...
  try {
    contextResponse =
      (await generateSearchResponseContext({
        query: originalQuery,
        results: filtered,
        filters: filters ?? {},
      })) || '';
