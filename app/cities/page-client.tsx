...
/**
 * Props for the CitiesPageClient component
 * Initial data is fetched server-side for faster loading
 */
export interface CitiesPageClientProps {
...
export default function CitiesPageClient({
  ...
  const hasSSRData = initialCityStats.length > 0;
  // Initialize state with SSR data if available
  const [cityStats, setCityStats] = useState<CityStats[]>(initialCityStats);
  const [filteredCities, setFilteredCities] = useState<CityStats[]>(initialCityStats);
  ...
  useEffect(() => {
    // Skip fetching if we have SSR data
