...
function mergeBySlug(primary: Destination[], secondary: Destination[]): Destination[] {
  if (primary.length === 0) {
    return [...secondary];
  }

  const seen = new Set(primary.map(destination => destination.slug));
...
export default function HomePageClient({ fallbackDestinations }: HomePageClientProps) {
  ...
  const initialFallback = useMemo(
    () => fallbackDestinations.map(destination => ({ ...destination })),
    [fallbackDestinations]
  );
  const initialFilters = useMemo(() => extractFilterOptions(initialFallback), [initialFallback]);

  const [destinations, setDestinations] = useState<Destination[]>(initialFallback);
  const [filteredDestinations, setFilteredDestinations] = useState<Destination[]>(initialFallback);
  ...
  const fallbackDestinationsRef = useRef<Destination[] | null>(initialFallback);
  ...
  useEffect(() => {
    fallbackDestinationsRef.current = initialFallback;
  }, [initialFallback]);

  const loadFallbackDestinations = async () => {
    if (fallbackDestinationsRef.current && fallbackDestinationsRef.current.length > 0) {
  ...
  const applyFallbackData = async (options: { updateDestinations?: boolean; ensureFilters?: boolean } = {}) => {
    ...
    if (updateDestinations) {
      setDestinations(prev => {
        const merged = prev.length ? mergeBySlug(prev, fallback) : [...fallback];
        return merged;
      });
      setFilteredDestinations(prev => {
        ...
        return mergeBySlug(prev, fallback);
      });
    }
  ...
  const fetchDestinations = useCallback(async () => {
    ...
    try {
      ...
      // Step 2: Show Supabase results immediately, merged with fallback entries when missing
      const supabaseDestinations = (Array.isArray(data) ? data : []) as Destination[];
      const fallbackForMerge = fallbackDestinationsRef.current ?? [];
      const supabaseWithFallback = mergeBySlug(supabaseDestinations, fallbackForMerge);

      setDestinations(supabaseWithFallback);

      // Extract unique cities and categories from merged data
      // IMPORTANT: Set cities/categories immediately (synchronously) to prevent empty filter flash
      const { cities: uniqueCities, categories: uniqueCategories } = extractFilterOptions(supabaseWithFallback);
      ...
      fetchDiscoveryBootstrap()
        .then((discoveryBaseline) => {
          if (discoveryBaseline.length > 0) {
            const supabaseWithDiscovery = mergeBySlug(supabaseDestinations, discoveryBaseline);
            const finalMerged = mergeBySlug(supabaseWithDiscovery, fallbackForMerge);

            // Only update if Discovery Engine provides additional value
            if (finalMerged.length > supabaseWithFallback.length) {
              setDestinations(finalMerged);
              const filteredWithDiscovery = filterDestinationsWithData(
                finalMerged,
                '', {}, '', '', user, visitedSlugs
              );
              setFilteredDestinations(filteredWithDiscovery);

              // OPTIMIZATION: Batch state updates - only update if Discovery Engine has more
              const { cities: discoveryCities, categories: discoveryCategories } = extractFilterOptions(finalMerged);
              const updates: { cities?: string[]; categories?: string[] } = {};
              if (discoveryCities.length > uniqueCities.length) {
                updates.cities = discoveryCities;
              }
