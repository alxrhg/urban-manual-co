...
export default function CollectionDetailPageClient({
  ...
  const [collection, setCollection] = useState<any>(initialCollection);
  ...
  const [editPublic, setEditPublic] = useState(initialCollection?.is_public ?? true);
  const [updating, setUpdating] = useState(false);

  const shouldSkipInitialFetchRef = useRef(Boolean(initialCollection));
  ...
  }, [collectionId, user?.id, authChecked, initialCollection]);
