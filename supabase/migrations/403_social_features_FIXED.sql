-- Social Features Database Schema (FIXED VERSION)
-- Handles missing dependencies and conflicts gracefully
...
CREATE TABLE IF NOT EXISTS user_profiles (
  ...
  favorite_cities TEXT[], -- Array of favorite cities
...
-- ============================================================================
-- LIST ITEMS TABLE (Destinations in lists)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_list_items_destination ON list_items(destination_slug);
...
CREATE INDEX IF NOT EXISTS idx_activities_destination ON activities(destination_slug) WHERE destination_slug IS NOT NULL;
...
-- List Items Policies
DROP POLICY IF EXISTS "List items viewable if list is viewable" ON list_items;
DROP POLICY IF EXISTS "Users can add items to their lists" ON list_items;
DROP POLICY IF EXISTS "Users can remove items from their lists" ON list_items;
...
DROP POLICY IF EXISTS "Helpful votes are viewable by everyone" ON review_helpful;
DROP POLICY IF EXISTS "Users can mark reviews as helpful" ON review_helpful;
DROP POLICY IF EXISTS "Users can remove their helpful votes" ON review_helpful;
...
-- Function for updating helpful count
...
-- Function to create activity when user saves a destination
-- Only create if saved_places table exists
DO $$
BEGIN
