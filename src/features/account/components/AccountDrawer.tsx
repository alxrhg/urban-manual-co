...
// Avatar with Progress Ring - modern gradient design
function AvatarWithRing({
  ...
  return (
    <div className="relative w-28 h-28 flex items-center justify-center">
      ...
      {/* Avatar */}
      <div className="w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 flex items-center justify-center shadow-inner relative">
        {avatarUrl ? (
          <Image
            src={avatarUrl}
            alt="Profile"
          ...
          <span className="text-3xl font-bold bg-gradient-to-br from-indigo-500 to-purple-500 bg-clip-text text-transparent">
...
// Travel Badge Component - modern gradient pill
function TravelBadge({ badge }: { badge: { name: string } }) {
  return (
    <span className="mt-3 px-4 py-1.5 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 dark:from-indigo-500/20 dark:to-purple-500/20 rounded-full text-xs font-semibold text-indigo-600 dark:text-indigo-400 backdrop-blur-sm">
      {badge.name}
    </span>
  );
}
...
// Library Tile Component - modern glass card style
function LibraryTile({
  ...
  const colorClasses = {
    indigo: 'from-indigo-500/10 to-indigo-500/5 hover:from-indigo-500/20 hover:to-indigo-500/10 dark:from-indigo-500/20 dark:to-indigo-500/10',
    emerald: 'from-emerald-500/10 to-emerald-500/5 hover:from-emerald-500/20 hover:to-emerald-500/10 dark:from-emerald-500/20 dark:to-emerald-500/10',
    amber: 'from-amber-500/10 to-amber-500/5 hover:from-amber-500/20 hover:to-amber-500/10 dark:from-amber-500/20 dark:to-amber-500/10',
    gray: 'from-gray-500/10 to-gray-500/5 hover:from-gray-500/20 hover:to-gray-500/10 dark:from-gray-500/20 dark:to-gray-500/10',
  };
  const iconColors = {
  ...
  return (
    <button
      onClick={onClick}
      className={`group flex flex-col items-center justify-center gap-2 p-5 rounded-2xl bg-gradient-to-b ${colorClasses[accentColor]} active:scale-[0.97] transition-all duration-200`}
    >
      <div className={`p-2.5 rounded-xl bg-white/60 dark:bg-white/5 shadow-sm`}>
      ...
      <div className="h-full flex flex-col bg-white dark:bg-gray-950">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex flex-col items-center px-5 pb-6 pt-2">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5 text-center truncate max-w-full">
              {user.email}
            </p>

            <button
              onClick={() => handleNavigate('/account')}
              className="mt-4 flex items-center gap-2 px-4 py-2 text-xs font-semibold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-all"
            >
          ...
          {/* Journey Progress - Modern gradient card */}
          <div className="px-5 mb-5">
            <div className="relative overflow-hidden p-5 rounded-2xl bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-pink-500/10 dark:from-indigo-500/20 dark:via-purple-500/15 dark:to-pink-500/10">
              ...
              <div className="relative">
                <div className="flex items-baseline gap-2 mb-3">
                  <span className="text-3xl font-bold text-gray-900 dark:text-white tabular-nums">{stats.visited}</span>
                  <span className="text-sm font-medium text-gray-600 dark:text-gray-400">places</span>
                  {stats.countries > 0 && (
                    <>
                      <span className="text-gray-300 dark:text-gray-600">·</span>
                      <span className="text-lg font-semibold text-gray-900 dark:text-white tabular-nums">{stats.countries}</span>
                      <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{stats.countries === 1 ? 'country' : 'countries'}</span>
                    </>
                  )}
                </div>
                <div className="h-2 bg-gray-200/80 dark:bg-gray-800/80 rounded-full overflow-hidden mb-2.5">
                  <div
          ...
          {/* Library Grid */}
          <div className="px-5 mb-4">
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-3 px-1">
              Your Library
            </h3>
