 ...
 * Initial data is fetched server-side and passed as props for SSR optimization
 */
export interface HomePageClientProps {
  /** Initial destinations from server-side fetch */
  initialDestinations: Destination[];
  /** Initial list of cities from server-side fetch */
  initialCities: string[];
  /** Initial list of categories from server-side fetch */
...
/**
 * HomePageClient - Client component that handles all interactive homepage features
 * Receives initial data from the server component for SSR optimization
 */
export function HomePageClient({
  ...
  // Initialize state with server-side data for instant display
  ...
  useEffect(() => {
    ...
    // SSR optimization: Only fetch if we don't have initial data from server
    // This provides a fallback for edge cases where SSR data might be empty
    if (initialDestinations.length === 0) {
      void fetchDestinations();
    }
    // fetchFilterData enhances cities/categories if Discovery Engine has more
    if (initialCities.length === 0 || initialCategories.length === 0) {
      void fetchFilterData();
    }
  }, [initialDestinations.length, initialCities.length, initialCategories.length]);
