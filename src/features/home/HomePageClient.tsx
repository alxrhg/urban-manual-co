...
import { TrendingSection, type TrendingDestination } from '@/components/TrendingSection';
...
import {
  slugify,
  normalizeDiscoveryEngineRecord,
  pickString,
  getFirstArrayItem,
  toNumberOrNull,
  toTrimmedString,
  isRecord,
} from '@/src/lib/data/records';
import { HomeHeroShell } from '@/src/features/home/components/HomeHeroShell';
import { HomeResultContainer } from '@/src/features/home/components/HomeResultContainer';
import { HomeDrawerPortal } from '@/src/features/home/components/HomeDrawerPortal';
import { HomeAdsRail } from '@/src/features/home/components/HomeAdsRail';
...
export function HomePageClient({
  ...
  const { user } = useAuth();
  const initialFilterOptions = useMemo(() => extractFilterOptions(initialDestinations), [initialDestinations]);
  const initialCities = initialFilterOptions.cities;
  const initialCategories = initialFilterOptions.categories;
  const { trackDestinationClick, trackFilterChange } = useHomePageAnalytics();
  ...
  useEffect(() => {
    if (initialDestinations.length) {
      ...
      setCities(initialFilterOptions.cities);
      setCategories(initialFilterOptions.categories);
    }
  }, [initialDestinations, initialCities, initialCategories]);
    ...
    <ErrorBoundary>
      ...
      <main id="main-content" className="relative min-h-screen dark:text-white" role="main">
              ...
              <div className="w-full md:w-1/2 md:ml-[calc(50%-2rem)] max-w-2xl flex flex-col h-full">
                <div className="flex-1 flex items-center">
                  <div className="w-full">
                    ...
                    {submittedQuery && (
                      <div className="text-xs uppercase tracking-[2px] text-gray-500 dark:text-gray-400 mt-6">
                        Showing AI-curated results for: <span className="text-black dark:text-white">{submittedQuery}</span>
                      </div>
                    )}
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="mt-6">
                          <div className="text-xs uppercase tracking-[2px] text-gray-500 dark:text-gray-400 mb-2">
        ...
        <HomeResultContainer>
            ...
            {!submittedQuery && trendingDestinations.length > 0 && (
              <div className="mb-12 md:mb-16">
