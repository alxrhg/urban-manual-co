...
import { IntentConfirmationChips } from './IntentConfirmationChips';
...
import { useSearchGridColumns } from '../hooks';
import { useAuth } from '@/contexts/AuthContext';
import type { Destination as DestinationType } from '@/types/destination';
import { generateSuggestions } from '../api';

const POIDrawer = dynamic(() => import('./POIDrawer').then(mod => ({ default: mod.POIDrawer })), {
  ssr: false,
});
...
function SearchPageContent() {
  ...
  const itemsPerPage = useSearchGridColumns(4); // Always 4 full rows
              ...
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4 md:gap-6 items-start">
                  {paginatedResults.map((d, idx) => {
                    const destinationForCard = d as DestinationType;
                    return (
                    <DestinationCard
                      key={d.id}
                      destination={destinationForCard}
                      onClick={() => router.push(`/destination/${destinationForCard.slug || destinationForCard.id}`)}
