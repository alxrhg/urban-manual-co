...
function AvatarWithRing({
  ...
  return (
    ...
    >
      <div className="w-16 h-16 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center relative">
        {avatarUrl ? (
          <Image
            src={avatarUrl}
            alt="Profile"
...
/**
 * AccountContent - User account view for IntelligentDrawer
 */
const AccountContent = memo(function AccountContent() {
        ...
        const uniqueCountries = new Set(
          (countriesResult.data || [])
            .map((item: Record<string, unknown>) => {
              ...
              if (Array.isArray(dest)) return dest[0]?.country;
  ...
  const handleOpenDestination = useCallback((destination: Destination) => {
    openDestinationWithTrip(destination);
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center px-8 py-12 text-center min-h-[400px]">
