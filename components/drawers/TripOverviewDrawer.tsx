...
export default function TripOverviewDrawer({ isOpen, onClose, trip: initialTrip }: TripOverviewDrawerProps) {
  ...
  if (!displayTrip && !loading) {
    return null;
  }

  const tripName = displayTrip ? ((displayTrip as any).name || displayTrip.title || 'Untitled Trip') : 'Loading...';
  const city = displayTrip?.destination || 'Unknown';
  const coverImage = displayTrip?.coverImage || displayTrip?.cover_image;

  const startDate = displayTrip ? formatTripDateWithYear(displayTrip.startDate || displayTrip.start_date) : '';
  const endDate = displayTrip ? formatTripDateWithYear(displayTrip.endDate || displayTrip.end_date) : '';
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="card-right" className="flex flex-col p-0" hideCloseButton>
        ...
        <div className="flex items-center gap-3 p-6 pb-4">
          <Button variant="ghost" size="icon" onClick={onClose} className="-ml-2 h-10 w-10">
        ...
        <ScrollArea className="flex-1">
          {loading ? (
            ...
            <div className="px-6 py-2 space-y-8 pb-20">
              {/* COVER IMAGE */}
              {coverImage && (
                ...
                <Card className="p-4 space-y-3">
                  ...
                  {allDestinations.length === 0 && (
                    <div className="flex flex-col items-center py-6 text-center">
                      <Compass className="w-8 h-8 text-gray-300 dark:text-gray-600 mb-2" />
