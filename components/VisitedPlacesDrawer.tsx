        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-safe">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 sm:py-16">
              <Loader2 className="w-8 h-8 sm:w-6 sm:h-6 animate-spin text-stone-300 dark:text-stone-600" />
              <p className="mt-4 text-base sm:text-sm text-stone-500 dark:text-stone-400">
            ...
            <div className="flex flex-col items-center justify-center py-20 sm:py-16 px-8 text-center">
              <div className="w-20 h-20 sm:w-16 sm:h-16 rounded-full bg-stone-100 dark:bg-stone-800 flex items-center justify-center mb-5">
                <MapPin className="w-9 h-9 sm:w-7 sm:h-7 text-stone-400 dark:text-stone-500" />
              </div>
              ...
              {visitedPlaces.map((place) => (
                place.destination && (
                  <div key={place.destination_slug} className="relative">
                    ...
                    />
                    {place.visited_at && (
                      <div className="absolute top-2.5 right-2.5 text-[11px] sm:text-[10px] font-medium text-stone-500 dark:text-stone-400 bg-white/90 dark:bg-stone-900/90 backdrop-blur-sm px-2 py-1 rounded-lg">
        ...
        {visitedPlaces.length > 0 && (
          <div className="px-5 sm:px-6 py-4 pb-safe border-t border-stone-100 dark:border-stone-900">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 sm:py-3 rounded-xl text-base sm:text-sm font-medium text-stone-600 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 active:bg-stone-200 dark:active:bg-stone-700 transition-colors min-h-[52px] sm:min-h-[44px]"
            >
              View full history
