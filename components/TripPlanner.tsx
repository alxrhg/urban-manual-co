...
import {
  ...
  DestinationInput,
  fetchTripDetails,
  fetchTripSummaries as fetchTripSummariesService,
...
export function TripPlanner({
  ...
  const fetchTripSummaries = useCallback(async () => {
    ...
    try {
      ...
      const summaries = await fetchTripSummariesService(user.id);
  ...
  const loadTrip = useCallback(async (id: string) => {
    ...
    try {
      const details = await fetchTripDetails(user.id, id);
      if (!details) return;

      const { trip, days: itineraryDays } = details;
  ...
  const handleAddDestinationToTrip = async (targetTripId: string) => {
    ...
    try {
      const destinationPayload: DestinationInput = {
      ...
      await appendDestinationToTrip(user.id, targetTripId, destinationPayload);
      await loadTrip(targetTripId);
      setCurrentTripId(targetTripId);
      setStep('plan');
      setActiveTab('itinerary');
      ...
      const message =
        error instanceof Error && error.message
          ? error.message
          : 'Failed to add destination to trip. Please try again.';
      alert(message);
    } finally {
  ...
  const handleSaveTrip = async () => {
    ...
    setSaving(true);
    try {
      await updateTrip(user.id, currentTripId, {
        title: tripName,
        description: hotelLocation || null,
        destination,
        start_date: startDate,
      ...
      await replaceItineraryItems(currentTripId, itemsToInsert);
  ...
  const baseTranslateClass = isDrawer
    ? isVisible
      ? 'translate-x-0'
      : 'translate-x-full'
    : '';
  const desktopTranslateClass = isDrawer
    ? isVisible
      ? 'md:translate-x-0'
  ...
  const panelClasses = isDrawer
    ? 'flex h-full w-full flex-col bg-white shadow-2xl dark:bg-gray-950 md:w-[440px] md:max-w-[calc(100vw-2rem)] md:rounded-2xl md:ring-1 md:ring-black/5 md:border md:border-gray-200 dark:md:border-gray-800 md:overflow-hidden'
    : 'flex w-full flex-col rounded-3xl border border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-950';
