...
/**
 * Universal Drawer Component - Native Feel
 *
 * Features:
...
export function Drawer({
  ...
  // Track if drawer has ever been opened (for exit animations)
  ...
  // Track when drawer opens for the first time
  useEffect(() => {
  ...
  const computedMobileHeight = mobileHeight ?? '96vh';
  const computedMobileMaxHeight = mobileMaxHeight ?? '100vh';
  const radiusClass = mobileBorderRadius ?? 'rounded-t-[28px]';

  // Don't render anything until first open (unless keepStateOnClose)
  if (!hasBeenOpened && !keepStateOnClose) return null;
      ...
      <div className={`flex-shrink-0 min-h-[3.5rem] px-6 flex items-center justify-between ${headerBackground} border-b border-black/5 dark:border-white/5 z-20`}>
        {headerContent || (
          <>
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 active:scale-95 transition-all"
              aria-label="Close"
            >
  ...
  return (
    <>
      ...
      {showBackdrop && (
        <div
          className={`fixed inset-0 bg-black/40 backdrop-blur-[2px] transition-opacity duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
      ...
      {mobileVariant === 'bottom' && (
        <div
          ...
          className={`
            ...
            ${backgroundClasses} w-full ${radiusClass}
          ...
          style={{
            ...
            height: isDragging ? undefined : computedMobileHeight,
          }}
          role="dialog"
          aria-modal="true"
        >
      ...
      <div
        ref={desktopRef}
        className={`
          hidden md:flex fixed ${desktopSpacing} rounded-2xl
          ${backgroundClasses} ${shadowClasses} ${borderClasses}
          flex-col overflow-hidden
          transition-transform duration-400 ease-[cubic-bezier(0.32,0.72,0,1)]
          ${isOpen
            ? 'translate-x-0'
            : position === 'right' ? 'translate-x-[calc(100%+2rem)]' : '-translate-x-[calc(100%+2rem)]'
          }
      ...
      {mobileVariant === 'side' && (
        <div
          ref={mobileSideRef}
          className={`
            md:hidden fixed top-0 bottom-0 w-full ${position === 'right' ? 'right-0' : 'left-0'}
