...
export default function AlertsDropdown({
  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      <button
        ...
        className={`
          ...
          ${hasWarnings
            ...
            : 'hover:bg-stone-100 dark:hover:bg-gray-800 opacity-50'
          }
        `}
        title={hasWarnings ? `${warnings.length} alerts` : 'No alerts'}
      >
        <Bell className={`w-5 h-5 ${highCount > 0 ? 'text-red-500' : 'text-stone-500 dark:text-gray-400'}`} />
        {hasWarnings && (
          <span className={`
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-stone-200 dark:border-gray-800 z-50 overflow-hidden">
          {/* Header */}
          ...
          <div className="max-h-80 overflow-y-auto">
            {warnings.length === 0 ? (
              <div className="p-6 text-center">
                <Bell className="w-8 h-8 mx-auto text-stone-300 dark:text-gray-700 mb-2" />
              ...
              <div className="divide-y divide-stone-100 dark:divide-gray-800">
                {warnings.map((warning) => {
                  ...
                  return (
                    <div
                      key={warning.id}
                      className={`p-4 ${config.bg} hover:bg-stone-50 dark:hover:bg-gray-800/50 transition-colors`}
                    >
